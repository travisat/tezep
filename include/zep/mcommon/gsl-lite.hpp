    ...
    namespace gsl
{
    ...
    using index = size_t; // p0122r3 uses std::ptrdiff_t
...
//
    ...
    constexpr auto gsl_HAVE_OWNER_TEMPLATE = gsl_HAVE_ALIAS_TEMPLATE;
...
#define Owner(t) ::gsl::owner<t>::type
    ...
    constexpr auto gsl_ELIDE_CONTRACT_EXPECTS = (0 == (gsl_CONFIG_CONTRACT_LEVEL_MASK & 0x01));
    constexpr auto gsl_ELIDE_CONTRACT_ENSURES = (0 == (gsl_CONFIG_CONTRACT_LEVEL_MASK & 0x10));
...
#define gsl_STRINGIFY_(x) #x
    ...
    public:
        ...
        gsl_api auto operator=(final_action const&) -> final_action& gsl_is_delete;
        gsl_api auto operator=(final_action &&) -> final_action& gsl_is_delete;
    ...
    template <class F>
    gsl_api inline auto finally(F const& action) gsl_noexcept->final_action<F>
    {
        return final_action<F>(action);
    }

    template <class F>
    gsl_api inline auto finally(F && action) gsl_noexcept->final_action<F>
    {
    ...
    gsl_api inline gsl_constexpr auto narrow_cast(U && u) gsl_noexcept->T
    {
    ...
    template <class T, class U>
    gsl_api inline auto narrow(U u)->T
    {
        T t = narrow_cast<T>(u);
    ...
    gsl_api inline gsl_constexpr14 auto at(std::array<T, N> & arr, size_t index)->T&
    {
        Expects(index < N);
        return arr[index];
    }
    ...
    gsl_api inline gsl_constexpr14 auto at(Container & cont, size_t index)->typename Container::value_type&
    {
    ...
    template <class T>
    gsl_api inline gsl_constexpr14 auto at(std::initializer_list<T> cont, size_t index)->T&
    {
        Expects(index < cont.size());
    ...
    template <class T>
    gsl_api inline gsl_constexpr auto at(span<T> s, size_t index)->T&
    {
        return s.at(index);
    }
...
#undef gsl_not_null_explicit
        ...
        gsl_api auto operator=(not_null&& other) -> not_null& = default;
        gsl_api auto operator=(not_null const& other) -> not_null& = default;
#else
        gsl_api ~not_null(){};
        ...
        gsl_api gsl_constexpr14 auto get() const -> get_result_t
        {
        ...
        gsl_api gsl_constexpr auto operator-> () const -> get_result_t
        {
            return get();
        }
        ...
        gsl_api auto operator=(std::nullptr_t) -> not_null& gsl_is_delete;
#else
            gsl_api
        ...
        gsl_api auto operator++() -> not_null& gsl_is_delete;
        gsl_api auto operator--() -> not_null& gsl_is_delete;
        gsl_api auto operator++(int) -> not_null gsl_is_delete;
        gsl_api auto operator--(int) -> not_null gsl_is_delete;
        gsl_api auto operator+(size_t) -> not_null& gsl_is_delete;
        gsl_api auto operator+=(size_t) -> not_null& gsl_is_delete;
        gsl_api auto operator-(size_t) -> not_null& gsl_is_delete;
        gsl_api auto operator-=(size_t) -> not_null& gsl_is_delete;
        gsl_api auto operator+=(std::ptrdiff_t) -> not_null& gsl_is_delete;
        gsl_api auto operator-=(std::ptrdiff_t) -> not_null& gsl_is_delete;
    ...
    template <class T, class U>
    auto operator-(not_null<T> const&, not_null<U> const&)->std::ptrdiff_t gsl_is_delete;

    template <class T>
    auto operator-(not_null<T> const&, std::ptrdiff_t)->not_null<T> gsl_is_delete;

    template <class T>
    auto operator+(not_null<T> const&, std::ptrdiff_t)->not_null<T> gsl_is_delete;

    template <class T>
    auto operator+(std::ptrdiff_t, not_null<T> const&)->not_null<T> gsl_is_delete;

    // not_null comparisons

    template <class T, class U>
    gsl_api inline gsl_constexpr auto operator==(not_null<T> const& l, not_null<U> const& r)->bool
    {
        return l.get() == r.get();
    }

    template <class T, class U>
    gsl_api inline gsl_constexpr auto operator<(not_null<U> const& l, not_null<U> const& r)->bool
    {
        return l.get() < r.get();
    }
...
#define gsl_ENABLE_IF_INTEGRAL_T(T)
    ...
    template <class T>
    gsl_api inline gsl_constexpr auto to_byte(T v) gsl_noexcept->byte
    {
#if gsl_HAVE(ENUM_CLASS_CONSTRUCTION_FROM_UNDERLYING_TYPE)
    ...
    gsl_api inline gsl_constexpr auto to_integer(byte b) gsl_noexcept->IntegerType
    {
#if gsl_HAVE(ENUM_CLASS_CONSTRUCTION_FROM_UNDERLYING_TYPE)
    ...
    gsl_api inline gsl_constexpr auto to_uchar(byte b) gsl_noexcept->unsigned char
    {
        return to_integer<unsigned char>(b);
    }

    gsl_api inline gsl_constexpr auto to_uchar(int i) gsl_noexcept->unsigned char
    {
    ...
    template <class IntegerType gsl_ENABLE_IF_INTEGRAL_T(IntegerType)>
    gsl_api inline gsl_constexpr14 auto operator<<=(byte& b, IntegerType shift) gsl_noexcept->byte&
    {
#if gsl_HAVE(ENUM_CLASS_CONSTRUCTION_FROM_UNDERLYING_TYPE)
    ...
    template <class IntegerType gsl_ENABLE_IF_INTEGRAL_T(IntegerType)>
    gsl_api inline gsl_constexpr auto operator<<(byte b, IntegerType shift) gsl_noexcept->byte
    {
    ...
    gsl_api inline gsl_constexpr14 auto operator|=(byte& l, byte r) gsl_noexcept->byte&
    {
#if gsl_HAVE(ENUM_CLASS_CONSTRUCTION_FROM_UNDERLYING_TYPE)
    ...
    gsl_api inline gsl_constexpr auto operator|(byte l, byte r) gsl_noexcept->byte
    {
    ...
    gsl_api inline gsl_constexpr auto operator~(byte b) gsl_noexcept->byte
    {
    ...
    {
        gsl_constexpr with_container_t() gsl_noexcept = default;
    };
    ...
    public:
        ...
        using value_type = typename details::remove_cv<T>::type;
...
#if gsl_HAVE(CONSTRAINED_SPAN_CONTAINER_CTOR)
        template <class Container, class = typename std::enable_if<details::can_construct_span_from<Container, element_type>::value>::type>
        ...
        template <class Container, class = typename std::enable_if<std::is_const<element_type>::value && details::can_construct_span_from<Container, element_type>::value>::type>
        ...
        template <class Container>
        gsl_api gsl_constexpr span(with_container_t /*unused*/, Container& cont)
            : first_(cont.size() == 0 ? gsl_nullptr : gsl_ADDRESSOF(cont[0]))
            , last_(cont.size() == 0 ? gsl_nullptr : gsl_ADDRESSOF(cont[0]) + cont.size())
        {
        }
        ...
        template <class Container>
        gsl_api gsl_constexpr span(with_container_t /*unused*/, Container const& cont)
            : first_(cont.size() == 0 ? gsl_nullptr : gsl_ADDRESSOF(cont[0]))
            , last_(cont.size() == 0 ? gsl_nullptr : gsl_ADDRESSOF(cont[0]) + cont.size())
        {
        }
...
#if gsl_HAVE(IS_DEFAULT)
        gsl_api gsl_constexpr14 auto operator=(span&&) gsl_noexcept -> span& = default;
        gsl_api gsl_constexpr14 auto operator=(span const&) gsl_noexcept -> span& = default;
#else
    ...
    template< class U > operator=();
        ...
        gsl_api gsl_constexpr14 auto first(index_type count) const gsl_noexcept -> span
        {
            Expects(0 <= count && count <= this->size());
            return span(this->data(), count);
        }

        gsl_api gsl_constexpr14 auto last(index_type count) const gsl_noexcept -> span
        {
            Expects(0 <= count && count <= this->size());
        ...
        gsl_api gsl_constexpr14 auto subspan(index_type offset, index_type count) const gsl_noexcept -> span
        {
            Expects(
                0 <= offset && offset <= this->size() && 0 <= count && count <= this->size() - offset);
            return span(this->data() + offset, count);
        }

        // 26.7.3.4 Observers [span.obs]

        gsl_api gsl_constexpr auto size() const gsl_noexcept -> index_type
        {
            return narrow_cast<index_type>(last_ - first_);
        }

        gsl_api gsl_constexpr auto size_bytes() const gsl_noexcept -> index_type
        {
            return size() * narrow_cast<index_type>(sizeof(element_type));
        }

        gsl_api gsl_constexpr auto empty() const gsl_noexcept -> bool
        {
            return size() == 0;
        }
        ...
        gsl_api gsl_constexpr auto operator[](index_type index) const -> reference
        {
            return at(index);
        }
    ...
    public:
        ...
        template <
            ...
            class = typename std::enable_if<
                ...
                && !details::is_basic_string_span<Container>::value
                && std::is_convertible<typename Container::pointer, pointer>::value
                && std::is_convertible<typename Container::pointer, decltype(std::declval<Container const&>().data())>::value>::type>
